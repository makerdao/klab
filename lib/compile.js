...
const prune_edges = (proofid, { prune, verbose } = {}) => {
  ...
  const logs = read(path.join(KLAB_WD_PATH, proofid + ".k.k.log"))
    .split("\n")
    .filter(l => !!l)
  // .map(l => l.split(" "))
  ...
  const rules = logs
    ...
    .reduce((a, ruleid) => {
      if(a && !a[ruleid]) {
        try { a[ruleid] = JSON.parse(read(path.join(KLAB_WD_PATH, `${proofid}.k.k_blobs`, ruleid + ".json"))) }
        catch(e) {console.log('bad json: ' + path.join(KLAB_WD_PATH, `${proofid}.k.k_blobs`, ruleid + ".json"))}
      }
      return a;
    }, {})
