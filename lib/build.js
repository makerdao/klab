...
const rules_template = rules => `requires "edsl.md"
...
const makeInterabiExhaustiveness = (alias, cname, contract, hasher) => {
  const if_not_entries = contract.abi
    ...
    .map(abiexpr => `andBool ${abiexpr} =/=Int #asWord(VCallData [ 0 .. 4 ])`)
    .join("\n")
  ...
  const spec = kjson.renderRule({
    ...
    term: {
      ...
      "ethereum.evm.callState.localMem"     : ".Memory => ?_",
      "ethereum.evm.callState.pc"           : "0 => ?_",
      "ethereum.evm.statusCode"             : "_ => EVMC_REVERT",
      "ethereum.evm.endPC"                  : "_ => ?_",
      "ethereum.evm.callState.gas"          : "#gas(VGas) => ?_",
...
const __getRawGas = name => read(path.join(KLAB_OUT, "gas", name + ".raw")).trim();
...
const getTerm = ({
  ...
  "ethereum.evm.callState.localMem"     : internal ? "_" : ".Memory => ?_",
...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, lemma}) => {
  ...
  const buildAccount = varname => {
    ...
    const storageName = varname + '_STORAGE';
    const origStorageName = varname + '_ORIG_STORAGE';
    // Set origStorage to V <- K, and restrict K to be in range of a uint256.
    const origStorage_str = (key) => {
      pre_storage_cond.push('#lookup(' + origStorageName + ', ' + storagekeys(realname(varname),key) + ') ==Int ' + remember_junk('_'));
      return '';
    }
    const storage_str = (key) => {
      orgVal = clean(act.storage[varname][key][0]).trim();
      newVal = clean(act.storage[varname][key][1]).trim();
      pre_storage_cond.push('#lookup(' + storageName + ', ' + storagekeys(realname(varname),key) + ') ==Int ' + remember_junk(orgVal));
      if (orgVal == newVal) {
        return '';
      }
      return '[ ' + storagekeys(realname(varname), key) + ' <- ' + newVal + ' ]';
    }
    ...
    for (i = 0; i < Object.keys(act.storage[varname]).length; i++) {
      keyi = storagekeys(realname(varname), Object.keys(act.storage[varname])[i]);
      for (j = i + 1; j < Object.keys(act.storage[varname]).length; j++) {
        keyj = storagekeys(realname(varname), Object.keys(act.storage[varname])[j]);
        pre_storage_cond.push(storagekeys(realname(varname), keyi) + ' =/=Int ' + storagekeys(realname(varname), keyj));
      }
    }

    var storage, origStorage;
    if(! pass) {
      storage     = storageName + ' => ?_' + storageName;
      origStorage = origStorageName;
    } else if (isCreate) {
      storage     = '.Map => .Map ' + storage_strs;
      origStorage = '.Map';
    } else {
      storage     = storageName + ' => ' + storageName + ' ' + storage_strs;
      origStorage = origStorageName;
    }
    ...
    const balance = act.balance
      && act.balance[varname]
      || varname + "_balance";
    const code = isCreate && isPass && `.ByteArray => ${alias}_bin_runtime` || isCreate && ".ByteArray => ?_" || alias + "_bin_runtime"
  ...
  // GAS
  const gasRewrite = "#gas(VGas) => " + ((hasGas && !oog) ? gas : "?_");
  let gasCond;
  if (hasGas && pass) {
      // TODO : oog?!
  ...
  const cond = pos_cond + '\n'
    + ( pre_storage_cond.length > 0 ? '\n  andBool ' + pre_storage_cond.join('\n  andBool ') : '\n' )
    + (neg_cond.length > 0 && `\n  andBool notBool (\n    ${ neg_cond_str }\n  )` || "")
...
const buildActs = (config, act_proofs) => {
  ...
  const forkPass = (obj, rule) => {
    const gas_raw = __getRawGas(rule.hash)
    let fork_obj = {
      ...obj,
      act: rule.act,
      pass: true,
    ...
    Object.keys(act_proofs)
      .forEach(name => {
        ...
        if (toClose) {
          ...
          // const gmt = (name, str) => `requires "data.md"\n\nmodule ${name}\n  imports EVM-DATA\n\n${str}\n\nendmodule`
