...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, lemma}) => {
  ...
  const buildAccount = varname => {
    ...
    const storage_str = (key) => {
      pre_storage_cond.push('#lookup(' + storageName + ', ' + storagekeys(realname(varname),key) + ') ==Int ' + remember_junk(clean(act.storage[varname][key][0])));
      return '[ ' + storagekeys(realname(varname), key) + ' <- ' + clean(act.storage[varname][key][1]) + ' ]';
    }
