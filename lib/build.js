...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, lemma}) => {
  ...
  const buildAccount = varname => {
    ...
    const origStorage_str = (key) => {
      let lhs = `#lookup(${act.subject}_OS, ${storagekeys(realname(varname), key)}) ==Int (Junk_${junk})`
      let rhs = `#lookup(?${act.subject}_OS, ${storagekeys(realname(varname), key)}) ==Int (Junk_${junk})`
      junk++
      act.if = (act.if || []).concat(`(${lhs})`)
      act.ensures = (act.ensures || []).concat(`(${rhs})`)
      return `[${isCreate ? "0" : lhs} ${rhs}]`
    }
    const storage_str = (key) => {
      let lhs = `#lookup(${act.subject}_S, ${storagekeys(realname(varname), key)}) ==Int (${remember_junk(clean(act.storage[varname][key][0]))})`
      let rhs = `#lookup(?${act.subject}_S, ${storagekeys(realname(varname), key)}) ==Int (${isPass ? clean(act.storage[varname][key][1]) : "?_"})`
      act.if = (act.if || []).concat(`(${lhs})`)
      act.ensures = (act.ensures || []).concat(`(${rhs})`)
    ...
    if(isCreate && pass) {
      ...
      storage = `        \n ${act.subject}_S => ?${act.subject}_S\n`;
      origStorage = `        \n ${act.subject}_OS => ?${act.subject}_OS\n`;
    }
